...
try{
	...
	if (rs.getString("pogodba") != null){
		...
		// Open record
		String strsql = "SELECT * FROM `kupci` WHERE 0 = 1";
		rs = stmt.executeQuery(strsql);
		rs.moveToInsertRow();
		...
		String srchfld = "'" + tmpfld + "'";
			...
			strsql = "SELECT * FROM `kupci` WHERE `sif_kupca` = '" + srchfld +"'";
		...
		// Field cena
		tmpfld = ((String) x_rok_placila).trim();
		if (!IsNumeric(tmpfld)) { tmpfld = null;}
		if (tmpfld == null) {
		...
		if (tmpfld == null) {
			...
			rs.updateFloat("analiza",Float.parseFloat(tmpfld));
		}
	...
	out.println("Morate spremeniti šifro kupca!!!");
}catch (SQLException ex){
	out.println(ex.toString());
}
%>
<%@ include file="header.jsp" %>
<p><span class="jspmaker">Dodaj v: kupci<br><br><a href="kupcilist.jsp">Nazaj na pregled</a></span></p>
...
<form onSubmit="return EW_checkMyForm(this);"  action="kupciadd.jsp" method="post">
<p>
