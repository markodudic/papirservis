...
if (searchwhere != null && searchwhere.length() > 0) {
	session.setAttribute("okolje_searchwhere", searchwhere);
	startRec = 1; // Reset start record counter (new search)
	session.setAttribute("okolje_REC", new Integer(startRec));
}else{
	if (session.getAttribute("okolje_searchwhere") != null)
		searchwhere = (String) session.getAttribute("okolje_searchwhere");
}
%>
<%
...
if (request.getParameter("cmd") != null && request.getParameter("cmd").length() > 0) {
	...
	if (cmd.toUpperCase().equals("RESET")) {
		...
		session.setAttribute("okolje_searchwhere", searchwhere);
		session.removeAttribute("okolje_OB");	
...
if (OrderBy != null && OrderBy.length() > 0) {
	if (session.getAttribute("okolje_OB") != null &&
		((String) session.getAttribute("okolje_OB")).equals(OrderBy)) { // Check if an ASC/DESC toggle is required
		if (((String) session.getAttribute("okolje_OT")).equals("ASC")) {
			session.setAttribute("okolje_OT", "DESC");
		}else{
			session.setAttribute("okolje_OT", "ASC");
		}
	}else{
		session.setAttribute("okolje_OT", "ASC");
	}
	session.setAttribute("okolje_OB", OrderBy);
	session.setAttribute("okolje_REC", new Integer(1));
}else{
	OrderBy = (String) session.getAttribute("okolje_OB");
	if (OrderBy == null || OrderBy.length() == 0) {
		OrderBy = DefaultOrder;
		session.setAttribute("okolje_OB", OrderBy);
		session.setAttribute("okolje_OT", DefaultOrderType);
	}
}
...
// Build SQL
String strsql = "SELECT * FROM `okolje`";
whereClause = "";
...
if (OrderBy != null && OrderBy.length() > 0) {
	strsql = strsql + " ORDER BY `" + OrderBy + "` " + (String) session.getAttribute("okolje_OT");
}
...
if (request.getParameter("start") != null && Integer.parseInt(request.getParameter("start")) > 0) {
	...
	if (IsNumeric(request.getParameter("pageno"))) {
		...
		startRec = ((Integer) session.getAttribute("okolje_REC")).intValue();
		if (startRec <= 0) {
	...
	if (session.getAttribute("okolje_REC") != null)
		startRec = ((Integer) session.getAttribute("okolje_REC")).intValue();
	if (startRec==0) {
		startRec = 1; //Reset start record counter
		session.setAttribute("okolje_REC", new Integer(startRec));
...
<p><span class="jspmaker">Pregled: okolje</span></p>
<form action="okoljelist.jsp">
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		...
		&nbsp;&nbsp;<a href="okoljelist.jsp?cmd=reset">Prikaži vse</a>
		</span></td>
	</tr>
...
<% if ((ewCurSec & ewAllowAdd) == ewAllowAdd) { %>
	<tr></tr>
	<tr><td><a href="okoljeadd.jsp">Dodaj nov zapis</a></td></tr>
<% } %>
</table>
<form method="post">
<table class="ewTable">
...
<a href="okoljelist.jsp?order=<%= java.net.URLEncoder.encode("koda","UTF-8") %>">EWC&nbsp;(*)<% if (OrderBy != null && OrderBy.equals("koda")) { %><span class="ewTableOrderIndicator"><% if (((String) session.getAttribute("okolje_OT")).equals("ASC")) { %>(^)<% }else if (((String) session.getAttribute("okolje_OT")).equals("DESC")) { %>(v)<% } %></span><% } %></a>
<%=(OrderBy != null && OrderBy.equals("koda")) ? "</b>" : ""%>
		</td>
		<td>
<%=(OrderBy != null && OrderBy.equals("material")) ? "<b>" : ""%>
<a href="okoljelist.jsp?order=<%= java.net.URLEncoder.encode("material","UTF-8") %>">Material&nbsp;(*)<% if (OrderBy != null && OrderBy.equals("material")) { %><span class="ewTableOrderIndicator"><% if (((String) session.getAttribute("okolje_OT")).equals("ASC")) { %>(^)<% }else if (((String) session.getAttribute("okolje_OT")).equals("DESC")) { %>(v)<% } %></span><% } %></a>
<%=(OrderBy != null && OrderBy.equals("material")) ? "</b>" : ""%>
		</td>
</tr>
<%
...
if (key != null && key.length() > 0) { 
	out.print("okoljeview.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
...
<td><span class="jspmaker"><a href="<% key =  rs.getString("koda"); 
if (key != null && key.length() > 0) { 
	out.print("okoljeedit.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
	out.print("javascript:alert('Invalid Record! Key is null');");
} %>">Spremeni</a></span></td>
<% } %>
...
<td><span class="jspmaker"><a href="<% key =  rs.getString("koda"); 
if (key != null && key.length() > 0) { 
	out.print("okoljeadd.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
	out.print("javascript:alert('Invalid Record! Key is null');");
} %>">Kopiraj</a></span></td>
<% } %>
...
<% if ((ewCurSec & ewAllowDelete) == ewAllowDelete) { %>
<% if (recActual > 0) { %>
<p><input type="button" name="btndelete" value="Izbriši izbrane" onClick="this.form.action='okoljedelete.jsp';this.form.submit();"></p>
<% } %>
<% } %>
</form>
<%
