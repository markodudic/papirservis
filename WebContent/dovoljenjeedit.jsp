<%@ page session="true" buffer="16kb" import="java.sql.*,java.util.*,java.text.*" errorPage="dovolljenjalist.jsp"%>
...
<p><span class="jspmaker">Spremeni : dovoljenja<br><br><a href="dovoljenjelist.jsp">Nazaj na pregled</a></span></p>
...
if (EW_this.x_ewc && !EW_hasValue(EW_this.x_ewc, "SELECT" )) {
            if (!EW_onError(EW_this, EW_this.x_ewc, "SELECT", "Napačan vnos - EWC koda"))
                return false; 
        }
return true;
}
...
<form onSubmit="return EW_checkMyForm(this);"  name="dovoljenjeedit" action="dovoljenjeedit.jsp" method="post">
<p>
...
String sqlwrk_x_sif_enote = "SELECT `sif_enote`, `naziv` FROM `enote`" + " ORDER BY `" + session.getAttribute("dovoljenje_prikaz_enota")  + "` ASC";
...
%><a href="<%out.print("dovoljenjeedit.jsp?key=" + x_id + "&prikaz_enota=sif_enote");%>">sif_enote</a>&nbsp;<a href="<%out.print("dovoljenjeedit.jsp?key=" + x_id + "&prikaz_enota=naziv");%>">naziv</a>
&nbsp;</td>
	</tr>
	<tr>
...
ResultSet rswrk_x_ewc = stmtwrk_x_ewc.executeQuery(sqlwrk_x_ewc);
	...
	while (rswrk_x_ewc.next()) {
		...
		x_ewcList += ">" + rswrk_x_ewc.getString("koda") + " " + tmpValue_x_ewc + "</option>";
		rowcntwrk_x_ewc++;
	}
rswrk_x_ewc.close();
rswrk_x_ewc = null;
...
%><a href="<%out.print("dovoljenjeedit.jsp?key=" + x_id + "&prikaz_okolje=koda");%>">koda</a>&nbsp;<a href="<%out.print("dovoljenjeedit.jsp?key=" + x_id + "&prikaz_okolje=material");%>">material</a>
&nbsp;</td>
	</tr>
</table>
<p>
