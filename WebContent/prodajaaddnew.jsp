...
try{
	...
	x_sif_enote = String.valueOf(rs.getLong("sif_enote"));
		...
		String strsqlGetKey = "select IFNULL(max(st_dob) + 1,1) mx from " + session.getAttribute("letoTabelaProdaja") + " prodaja";
		rs = stmt.executeQuery(strsqlGetKey );
		if (!rs.next()){
			rs.close();
			rs = null;
			stmt.close();
		...
		// Get the field contents
		x_st_dob = String.valueOf(rs.getLong("mx"));
...
String sqlwrk_x_koda = "SELECT `materiali`.`koda`, `materiali`.`material`, material_okolje.okolje_koda " +
						"FROM `materiali` " +
...
function updateKoda(EW_this){
	if (sif_ewc[document.prodajaaddnew.x_koda.selectedIndex-1] != "null")
		document.prodajaaddnew.x_ewc_ll.value = sif_ewc[document.prodajaaddnew.x_koda.selectedIndex-1];
	else
		document.prodajaaddnew.x_ewc_ll.selectedIndex = 0;
}
...
function put(){
	document.prodajaaddnew.x_kol_p.value = document.prodajaaddnew.x_kol_n.value;
}



</script>
<form onSubmit="return EW_checkMyForm(this);"  action="prodajaadd.jsp" method="post" name="prodajaaddnew">
<p>
...
<table class="ewTable">
	...
	<tr>
		...
		<td class="ewTableAltRow"><%out.println(x_sif_kupcaList);%>&nbsp;<span class="jspmaker"><a href="<%out.print("prodajaaddnew.jsp?prikaz_kupca=sif_kupca");%>">šifra</a>&nbsp;<a href="<%out.print("prodajaaddnew.jsp?prikaz_kupca=naziv");%>">naziv</a>&nbsp;<a href="<%out.print("prodajaaddnew.jsp?prikaz_kupca=naslov");%>">naslov</a>&nbsp;</a></span>&nbsp;</td>
	</tr>
	<tr>
		<td class="ewTableHeader">Koda&nbsp;</td>
		<td class="ewTableAltRow"><%out.println(x_kodaList);%>
		<a href="<%out.print("prodajaaddnew.jsp?prikaz_material=koda");%>">koda</a>&nbsp;<a href="<%out.print("prodajaaddnew.jsp?prikaz_material=material");%>">material</a>&nbsp;</td>
	</tr>
	<tr>
		<td class="ewTableHeader">EWC&nbsp;</td>
		<td class="ewTableAltRow"><%out.println(x_ewcList);%>&nbsp;
		<a href="<%out.print("prodajaaddnew.jsp?prikaz_okolje=koda");%>">ewc</a>&nbsp;<a href="<%out.print("prodajaaddnew.jsp?prikaz_okolje=material");%>">material</a></td>
	</tr>	
	<tr>
	...
	<tr>
		...
		<td class="ewTableAltRow"><input type="text"  onblur = "put()" name="x_kol_n" size="30" value="<%= HTMLEncode((String)x_kol_n) %>">&nbsp;</td>
	</tr>
	<tr>
