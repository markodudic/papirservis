...
try{
	...
	x_uporabnik = String.valueOf(rs.getLong("uporabnik"));
		...
		// Open record
		String strsql = "insert into osnovna (sif_os,osnovna,cena_am,kol_sk,kol_mb,kol_nm,kol_dv, uporabnik) values(";
		...
		String srchfld = "'" + tmpfld + "'";
			...
			String strsql11 = "SELECT * FROM `osnovna` WHERE `sif_os` = '" + srchfld +"'";
			Statement stmtchk = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
			ResultSet rschk = stmtchk.executeQuery(strsql11);
			if (rschk.next()) {
				out.print("Duplicate key for sif_os, value = " + tmpfld + "<br>");
...
<p><span class="jspmaker">Dodaj v: osnovna<br><br><a href="osnovnalist.jsp">Nazaj na pregled</a></span></p>
...
if (EW_this.x_cena_am && !EW_checknumber(EW_this.x_cena_am)) {
            if (!EW_onError(EW_this, EW_this.x_cena_am, "TEXT", "Napačna številka - cena am"))
                return false; 
        }
if (EW_this.x_cena_am && !EW_checkinteger(EW_this.x_cena_am.value)) {
        if (!EW_onError(EW_this, EW_this.x_cena_am, "TEXT", "Napačna številka - cena am"))
            return false; 
        }
...
<form onSubmit="return EW_checkMyForm(this);"  action="osnovnaadd.jsp" method="post">
<p>
